...
function doInline(content, file, ext, type) {
    ...
    if (type === 'css') {
        content = ld + 'style id="' + inline + '"' + rd + fis.compile.extCss('@import url("' + inline + '?__inline")') +
            ld + 'endstyle' + rd + content;
    }
    else if (type === 'js') {
        content = ld + 'script id="' + inline + '"' + rd + fis.compile.extJs('__inline("' + inline + '")') + ld +
            'endscript' + rd + content;
    }
    return content;
}
